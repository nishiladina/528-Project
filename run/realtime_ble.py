#!/usr/bin/env python3
"""
Real-time IMU Gesture Recognition Pipeline — Bluetooth LE
=========================================================
Reads a 6-axis IMU stream from an ESP32 over BLE (Nordic UART Service),
detects gesture windows with an energy-based trigger, extracts features,
and classifies with the trained MLP model — all in near real-time.

Usage:
    python realtime_ble.py                          # connect to 'IMU-Stream'
    python realtime_ble.py --device "IMU-Stream"    # explicit device name
    python realtime_ble.py --demo                   # simulation, no hardware

Then open http://127.0.0.1:5001 in a browser.

Dependencies (beyond the training script):
    pip install flask bleak numpy scipy scikit-learn
"""
...
# ── BLE constants ──────────────────────────────────────────────────────────────
BLE_DEVICE_NAME  = "IMU-Stream"
NUS_TX_UUID      = "6e400003-b5a3-f393-e0a9-e50e24dcca9e"  # notify characteristic
LINE_RE          = re.compile(
...
class BleReader(threading.Thread):
    """
    Connects to an ESP32 BLE device advertising as BLE_DEVICE_NAME,
    subscribes to the NUS TX characteristic, and pushes parsed IMU rows
    to data_queue. Runs an asyncio event loop in a background thread.
    Also supports demo/simulation mode when no BLE device is available.
    """

    def __init__(self, device_name: str, data_queue: queue.Queue,
                 demo: bool = False):
        ...
        self.error       = None
        self._ble_buf    = ""          # partial-line accumulator
    ...
    async def _run_ble(self):
        ...
        try:
            ...
            self.error = f"BLE scan failed: {e}"
            return

        if device is None:
            self.error = (f"Could not find '{self.device_name}'. "
        ...
        try:
            async with BleakClient(device) as client:
                self.connected = True
                print("BLE connected. Subscribing to IMU stream.")
                await client.start_notify(NUS_TX_UUID, self._notification_handler)
                while self.running:
    ...
    def _notification_handler(self, sender, data: bytearray):
        """Called by bleak on each BLE notification packet."""
        self._ble_buf += data.decode("utf-8", errors="ignore")
        while "\n" in self._ble_buf:
            line, self._ble_buf = self._ble_buf.split("\n", 1)
            line = line.strip()
    ...
    def _run_demo(self):
        ...
        while self.running:
            if int(t * FS) % 300 == 0 and t > 0.5:
                ...
                for row in self._make_gesture_burst(gesture, rng):
                    if not self.running:
                        return
                    self.queue.put((time.time(), row))
                    time.sleep(dt)
    ...
    def _make_gesture_burst(gesture: str, rng) -> list[np.ndarray]:
        """Return ~150 samples that look like a gesture motion."""
        n   = 150
        t_  = np.linspace(0, 1, n)
        env = np.sin(np.pi * t_) * 0.6
        profiles = {
...
def api_status():
    return jsonify({
        ...
        "demo":       ble_reader.demo if ble_reader else False,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Real-time IMU Gesture Recognition — BLE")
    parser.add_argument("--device", default=BLE_DEVICE_NAME,
                        help=f"BLE device name to connect to (default: '{BLE_DEVICE_NAME}')")
    parser.add_argument("--demo",   action="store_true",
                        help="Run in demo/simulation mode (no BLE hardware needed)")
    ...
    # ── BLE reader ─────────────────────────────────────────────────────────────
    ble_reader = BleReader(args.device, data_queue, demo=args.demo)
    ble_reader.start()

    if args.demo:
        print("✓ Demo mode active — simulated gestures every ~3 s")
    else:
        print(f"✓ Connecting to BLE device '{args.device}' ...")
